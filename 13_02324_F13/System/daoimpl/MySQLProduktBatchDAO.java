...
public class MySQLProduktBatchDAO implements IProduktBatchDAO{
	...
	public ProduktBatchDTO getProduktBatch(int pbId) throws DALException {
		ResultSet rs = Connector.doQuery("SELECT pb.*,r.*,u.* FROM recept r, produktbatch pb, operatoer u WHERE r.recept_id=pb.recept_id AND u.opr_id = pb.opr_id AND pb_id = " + pbId);
		try {
	...
	public ArrayList<ProduktBatchDTO> getProduktBatchList() throws DALException {
		...
		ResultSet rs = Connector.doQuery("SELECT pb.*,r.*,u.* FROM recept r, produktbatch pb, operatoer u WHERE r.recept_id=pb.recept_id AND u.opr_id = pb.opr_id");
		try
		{
			while (rs.next()) 
			{
